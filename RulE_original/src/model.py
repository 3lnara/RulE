...
class RulE(torch.nn.Module):
    def __init__(self, graph, p_norm, mlp_rule_dim, gamma_fact, gamma_rule, hidden_dim, device, dataset,
        ...
        # Learnable per-relation beta: beta*rule_score + (1-beta)*kge_score
        self.beta = nn.Parameter(torch.zeros(self.num_relations * 2))
        nn.init.constant_(self.beta, 0.0)

        # Query-adaptive beta: sigmoid((beta[r] or global_beta) + beta_feature*feature(h,r)).
        self.beta_feature = nn.Parameter(torch.zeros(1))
        self.global_beta = nn.Parameter(torch.zeros(1))
        self.use_per_relation = use_per_relation
        self.feature_name = feature_name

        # Optional z-scoring of the query feature; stats from the beta-train split.
        ...
        # Per-query feature stashes, set by forward() / update_kge_stash() / eval_compute_rule_confidence().
    ...
    def _load_from_state_dict(self, state_dict, prefix, *args, **kwargs):
        """Remap the legacy beta_density key to beta_feature for old checkpoints."""
        legacy = prefix + 'beta_density'
        current = prefix + 'beta_feature'
    ...
    def forward(self, all_h, all_r, edges_to_remove):
        ...
        # Per-query count of distinct rules that fired.
        rules_fired = torch.zeros(all_h.size(0), device=device)

        # Top rule confidence; only when eval_compute_rule_confidence() ran.
        ...
        for index, (r_head, r_body) in self.relation2rules[query_r]:
            ...
            mask += count
            # Rule fired for query b iff it grounded to >=1 tail; reduce over entities.
            fired = (count.sum(dim=-1) > 0).float()
            rules_fired += fired

            if have_conf:
                top_conf = torch.maximum(top_conf, self.rule_confidence[index] * fired)
        ...
        # Fraction of this relation's rules that fired per query (in [0, 1]).
        self._last_num_rules = rules_fired / max(num_rules_total, 1)

        # Max rule confidence among fired rules per query; None if not precomputed.
        self._last_top_rule_conf = top_conf if have_conf else None

        if mask.sum().item() == 0:
            # No rule fired in this batch: stash an all-False grounding mask.
            self._last_ground_mask = torch.zeros_like(mask, dtype=torch.bool)
            return mask + self.bias.unsqueeze(0), (1 - mask).bool()

        # Pre-bias grounding mask: True iff >=1 rule fired on (query, entity).
    ...
    def eval_compute_rule_weight(self,device):
        '''Precompute per-rule message embeddings from rule/relation embeddings.'''
        batch = 128
    ...
    def eval_compute_rule_confidence(self, device):
        '''Precompute per-rule confidence sigmoid(RulE plausibility) in (0, 1);
        call once after load. Stored as self.rule_confidence.'''
        batch = 128
        self.rule_masks = self.rule_masks.to(device)
        self.rule_features = self.rule_features.to(device)
    ...
    def update_kge_stash(self, kge_score, flag):
        """Stash _last_kge_max / _last_kge_entropy over filtered candidates (call after compute_g_KGE())."""
        ...
        # Shannon entropy of softmax over filtered candidates; 0 when none.
    ...
    def get_query_feature(self):
        """Raw per-query feature [batch] per self.feature_name (density/num_rules/kge_max/kge_entropy/top_rule_confidence)."""
        if self.feature_name == 'kge_max':
            if self._last_kge_max is None:
                raise RuntimeError("kge_max feature requires update_kge_stash() first.")
            return self._last_kge_max
        if self.feature_name == 'kge_entropy':
            if self._last_kge_entropy is None:
                raise RuntimeError("kge_entropy feature requires update_kge_stash() first.")
            return self._last_kge_entropy
        if self.feature_name == 'top_rule_confidence':
            if self._last_top_rule_conf is None:
                raise RuntimeError("top_rule_confidence feature requires eval_compute_rule_confidence() first.")
            return self._last_top_rule_conf
        if self.feature_name == 'num_rules':
            if self._last_num_rules is None:
                raise RuntimeError("num_rules feature requires forward() first.")
            return self._last_num_rules
        if self._last_ground_mask is None:
            raise RuntimeError("density feature requires forward() first.")
        return self._last_ground_mask.float().mean(dim=-1)   # [batch]

    def compute_adaptive_beta(self, rule_score, all_r):
        """Query-adaptive beta = sigmoid(intercept + beta_feature*feature); returns (beta [batch,1], raw feature). rule_score unused."""
        ...
        logit = intercept + self.beta_feature * feat
